...
const BASE_URL =
  process.env.ZILLOW_VERIFY_MARKET || "https://www.zillow.com/oh/";
...
async function main(): Promise<void> {
  ...
  for (const page of PAGES) {
    ...
    if (!html) {
      logger.warn(`[verify] page ${page}: fetch failed/blocked`);
      continue;
    }
    ...
    logger.info(
      `[verify] page ${page}: listResults=${list.length} mapResults=${map.length}`,
    );
    ...
    if (pageDays.length > 0) {
      pageRanges.set(page, {
        min: Math.min(...pageDays),
        max: Math.max(...pageDays),
        n: pageDays.length,
      });
    }
  }
  ...
  for (const page of PAGES) {
    const pageItems = items.filter(
      (i) => i.page === page && i.bucket === "list" && i.days != null,
    );
  ...
  logger.info(
    `[verify] Within-page ordering violations: ${withinViolations}${withinViolations === 0 ? " ✓ (listResults ascending in days = Zillow newest-first)" : ""}`,
  );
  ...
  for (let i = 0; i < sortedPages.length - 1; i++) {
    const older = sortedPages[i][1],
      newer = sortedPages[i + 1][1];
    logger.info(
    ...
    if (older.max < newer.max) {
      crossViolations++;
      logger.warn(
        `[verify] CROSS-PAGE VIOLATION: older page max (${older.max}) < newer page max (${newer.max})`,
      );
    }
  }
  logger.info(
    `[verify] Cross-page violations: ${crossViolations}${crossViolations === 0 ? " ✓ (older pages hold older listings)" : ""}`,
  );
  ...
  logger.info(
    `[verify] Total items: ${items.length}, duplicate zpids: ${dupes}`,
  );
  ...
  if (anomalies === 0) {
    logger.info(
      `[verify] PASS — Zillow results are ordered correctly (within-page newest-first, pages oldest-first).`,
    );
  } else {
    logger.warn(
      `[verify] FAIL — ${anomalies} ordering anomaly(ies) found. Review the violations above.`,
    );
  }
}
