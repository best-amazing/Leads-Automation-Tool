    ...
    async () => {
      while (idx < tasks.length) {
        ...
        try {
          ...
          logger.warn(
            `[craigslist-adu] Pool task ${taskIndex} failed: ${err instanceof Error ? err.message : err}`,
          );
        }
      }
    },
...
export class CraigslistAduScraper {
  ...
  async run(): Promise<RawListing[]> {
    logger.info(
      `[${this.sourceName}] Starting ADU research scrape via Craigslist (concurrency=${DETAIL_CONCURRENCY})`,
    );
    this.visited.clear();
    this.results = [];
    ...
    for (const [cityName, baseUrl] of Object.entries(craigslistSources)) {
      ...
      if (cityState && !TARGET_STATES.includes(cityState)) {
        logger.info(
          `[${this.sourceName}] ${cityName} → ${cityState} not in target states (${TARGET_STATES.join(", ")}), skipping city`,
        );
        continue;
      }
      ...
      while (!stopPaging && processedThisBatch < BACKFILL_BATCH_SIZE) {
        ...
        logger.info(
          `[${this.sourceName}] ${cityName} — fetching offset ${offset}`,
        );
        ...
        if (rawListings.length === 0) {
          logger.info(
            `[${this.sourceName}] ${cityName} — no listings on offset ${offset}, stopping pagination`,
          );
          break;
        }
        ...
        const pendingDetails: Array<{
          rawListing: Omit<RawListing, "source">;
        ...
        if (newOnPage === 0) {
          logger.info(
            `[${this.sourceName}] ${cityName} — no new listings on offset ${offset}, stopping.`,
          );
          stopPaging = true;
        }
