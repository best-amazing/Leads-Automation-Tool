      ...
      (res: http.IncomingMessage) => {
        ...
        res.on("end", () => {
          try {
            ...
            if (!result?.IdToken) {
              logger.error(
                `[cl] Token refresh unexpected response: ${text.slice(0, 300)}`,
              );
              resolve(false);
              return;
            }
    ...
    const req = https.request(
      ...
      (res: http.IncomingMessage) => {
        ...
        res.on("end", async () => {
          ...
          logger.debug(
            `[cl] API response: HTTP ${res.statusCode} | ${text.length} chars`,
          );
...
function dealToRawListing(deal: CLDeal): RawListing {
  ...
  if (lt.includes("single") || lt.includes("family"))
  ...
  override async run(): Promise<RawListing[]> {
    ...
    if (CL_REFRESH_TOKEN) {
      ...
      if (!refreshed) {
        if (!clTokens.authToken) {
          logger.error(
            "[cl] Refresh failed and no fallback CL_AUTH_TOKEN — aborting run",
          );
          return [];
        }
        logger.warn(
          "[cl] Token refresh failed — continuing with existing CL_AUTH_TOKEN (may be expired)",
        );
      }
    } else if (!clTokens.authToken) {
      logger.error(
        "[cl] No CL_AUTH_TOKEN and no CL_REFRESH_TOKEN — aborting run",
      );
      return [];
    }
    ...
    for (const market of this.markets) {
      ...
      logger.info(
        `[cl] Scraping market: ${market.name} (state=${market.stateAbbr})`,
      );
        ...
        for (const deal of deals) {
          ...
          if (deal.purchasePrice != null) {
            if (deal.purchasePrice < config.filter.minPrice) {
              ...
              logger.debug(
                `[cl] ✗ Below min price: ${deal.streetAddress} @ $${deal.purchasePrice}`,
              );
              continue;
            }
            if (deal.purchasePrice > maxPrice) {
              ...
              logger.debug(
                `[cl] ✗ Above max price: ${deal.streetAddress} @ $${deal.purchasePrice}`,
              );
              continue;
            }
          }
