  ...
  return new Promise((resolve) => {
    ...
    const req = https.request(
      ...
      (res: http.IncomingMessage) => {
        ...
        logger.debug(
          `[zillow-enricher] [sock] response headers — status=${res.statusCode ?? 0} ` +
        ...
        (stream as NodeJS.ReadableStream).on("data", (c: Buffer) => {
          ...
          if (chunkCount === 1 || chunkCount % 100 === 0) {
            logger.debug(`[zillow-enricher] [sock] chunk #${chunkCount} (+${c.length} B, total ${totalBytes} B)`);
          }
        });
        (stream as NodeJS.ReadableStream).on("end", () => {
          logger.debug(`[zillow-enricher] [sock] end — total ${totalBytes} B`);
    ...
    const deadline = setTimeout(() => {
      logger.warn(`[zillow-enricher] Oxylabs deadline exceeded (${REQUEST_TIMEOUT_MS / 1_000}s) — aborting`);
    ...
    req.on("close", () => { clearTimeout(deadline); logger.debug("[zillow-enricher] [sock] close"); resolve(null); });
    req.write(bodyStr);
    req.end();
    logger.debug(`[zillow-enricher] [sock] request sent — ${targetUrl}`);
  });
}
