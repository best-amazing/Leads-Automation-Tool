...
import {
  upsertMany,
  upsertZillowListings,
  upsertRedfinListings,
  upsertRealtorListings,
  upsertPropwireListings,
  getSummaryStats,
  upsertPropertyFromEnrichment,
  upsertEstimateFromZillow,
  upsertEstimateFromRedfin,
  upsertEstimateFromPropwire,
  upsertSingleListing,
  updateListingPropertyId,
  getOldListingsWithoutPropertyLink,
} from "./db/repository";
...
function scoreListings(
  listings: RawListing[],
): Array<ListingUpsertPayload & { estimate?: number }> {
  return listings.map(
    (listing): ListingUpsertPayload & { estimate?: number } => {
...
export interface RunOptions {
  ...
  factories: Record<
    string,
    () => import("./scrapers/base.scraper").BaseScraper
  >;
...
export async function runScrapers(options: RunOptions): Promise<void> {
  ...
  for (const key of sourceKeys) {
    ...
    if (rawListings.length === 0) {
      logger.warn(
        `[${key}] No fresh listings from scraper — checking for old unenriched listings`,
      );
    ...
    try {
      ...
      if (oldDbListings.length > 0) {
        logger.info(
          `[${key}] Found ${oldDbListings.length} old unenriched listings from database`,
        );
      }
    } catch (err) {
      logger.warn(
        `[${key}] Could not fetch old listings for re-enrichment: ${err}`,
      );
    }
    ...
    const enrichmentConcurrency = parseInt(
      process.env.ZILLOW_ENRICHMENT_CONCURRENCY || "2",
    );
    logger.info(`\n${"─".repeat(60)}`);
    logger.info(`[${key}] Starting Zillow enrichment phase`);
    logger.info(
      `[${key}] Input: ${rawListings.length} fresh + ${oldDbListings.length} old = ${allListings.length} total listings`,
    );
    logger.info(`[${key}] Enrichment concurrency: ${enrichmentConcurrency}`);
    logger.info(`${"─".repeat(60)}\n`);

    // Enrich with Zillow
    let enrichedListings = await zillowEnrichmentService.enrichAllListings(
      allListings,
    ...
    logger.info(`[${key}] Enrichment concurrency: ${enrichmentConcurrency}`);
    logger.info(`${"─".repeat(60)}\n`);
    enrichedListings = await redfinEnrichmentService.enrichAllListings(
      enrichedListings,
      enrichmentConcurrency,
    );

    // Then enrich with Propwire
    const propwireConcurrency = parseInt(
      process.env.PROPWIRE_ENRICHMENT_CONCURRENCY || "1",
    );
    logger.info(`\n${"─".repeat(60)}`);
    logger.info(`[${key}] Starting Propwire enrichment phase`);
    logger.info(`[${key}] Enrichment concurrency: ${propwireConcurrency}`);
    logger.info(`${"─".repeat(60)}\n`);
    enrichedListings = await propwireEnrichmentService.enrichAllListings(
      enrichedListings,
      propwireConcurrency,
    );
    ...
    try {
      ...
      for (const payload of payloads) {
        try {
          ...
          logger.warn(
            `[${key}] Error processing listing ${payload.url}: ${err}`,
          );
        }
      }
      ...
      const zilowEnrichedCount = enrichedListings.filter(
        (l) => l.zestimate != null,
      ).length;
      const redfinEnrichedCount = enrichedListings.filter(
        (l) => l.redfinEstimate != null,
      ).length;
      const propwireEnrichedCount = enrichedListings.filter(
        (l) => l.propwireEstimate != null,
      ).length;
      logger.info(`\n${"─".repeat(60)}`);
      logger.info(`[${key}] Enrichment phase complete:`);
      logger.info(
        `[${key}]   • Fresh listings processed: ${rawListings.length}`,
      );
      logger.info(
        `[${key}]   • Old listings re-enriched: ${oldDbListings.length}`,
      );
      ...
      logger.info(
        `[${key}]   • Property+Estimate records created: ${propertiesCreated}`,
      );
      logger.info(`${"─".repeat(60)}\n`);
